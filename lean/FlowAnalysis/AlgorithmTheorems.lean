...
lemma split_chain_at_index {i : Nat} {c : PromotionChain} (h : i < c.val.length) :
  ...
  exists by
    apply isPromotionChain.lt_of_mem_tail
...
theorem joinPromotedTypes_correct (c₁ c₂ : PromotionChain) :
    (joinPromotedTypesA c₁.val c₂.val : Id _).run = (c₁.join c₂).val := by
  symm; apply Id.of_wp_run_eq (prog := (joinPromotedTypesA c₁.val c₂.val : Id _))
  · rfl
