...
-/
def joinPromotedTypesA' {m} [Monad m] (ts₁ ts₂ : List τ) : m (List τ) := do
  let mut i₁ := 0
  let mut i₂ := 0
  let mut T₁ := ts₁[0]!
  let mut T₂ := ts₂[0]!
...
public def joinPromotedTypesA {m} [Monad m] (ts₁ ts₂ : List τ) : m (List τ) := do
  let mut ts₁ := ts₁
  let mut ts₂ := ts₂
...
/--
Helper theorem for proving the correctness of `PromotionModel.joinPromotedTypes:
...
/-- Lemma establishing the key relation in the recursive definition of `join`. -/
@[simp]
...
/-- Justifies the recursive definition of `join` when `T₁ = T₂`. -/
@[simp]
theorem join.heads_eq {T : τ} {ts₁' ts₂' : List τ}
    {hvalid₁ : isPromotionChain (T::ts₁')} {hvalid₂ : isPromotionChain (T::ts₂')} :
...
/-- Justifies the recursive definition of `join` when `T₁ ≤ T₂`. -/
@[simp]
theorem join.heads_ne_le {T₁ T₂ : τ} {ts₁' ts₂' : List τ}
    {hvalid₁ : isPromotionChain (T₁::ts₁')} {hvalid₂ : isPromotionChain (T₂::ts₂')}
    (hne : ¬T₁ = T₂) (hle : T₁ ≤ T₂) :
    join ⟨T₁::ts₁', hvalid₁⟩ ⟨T₂::ts₂', hvalid₂⟩ =
...
/-- Justifies the recursive definition of `join` when `¬T₁ ≤ T₂`. -/
@[simp]
theorem join.heads_ne_not_le {T₁ T₂ : τ} {ts₁' ts₂' : List τ}
    {hvalid₁ : isPromotionChain (T₁::ts₁')} {hvalid₂ : isPromotionChain (T₂::ts₂')}
    (hne : ¬T₁ = T₂) (hnotLe : ¬T₁ ≤ T₂) :
    join ⟨T₁::ts₁', hvalid₁⟩ ⟨T₂::ts₂', hvalid₂⟩ =
...
/-- Specification (and correctness proof) for `joinPromotedTypesA'`. -/
theorem joinPromotedTypesA'_correct [Monad m] [Lean.Order.MonadTail m]
...
/-- Specification (and correctness proof) for `joinPromotedTypesA`. -/
public theorem joinPromotedTypesA_correct [Monad m] [Lean.Order.MonadTail m]
    [WPMonad m ps] (c₁ c₂ : PromotionChain) :
    ⦃stateIs s₀⦄ (joinPromotedTypesA c₁.val c₂.val : m _)
    ⦃⇓ r => stateIs s₀ ∧ ⌜r = (c₁.join c₂).val⌝⦄ := by
  unfold joinPromotedTypesA
